...
def get_city_detail():
    city_name = 'Hong Kong'
    lat_top_left = '22.252395'
    lng_top_left = '113.960305'
    lat_bottom_right = '22.209818'
    lng_bottom_right = '114.155305'
...
def save_to_csv(data):
    ...
    if data['resto_id'] not in count_list:
          count_list.append(data['resto_id'])
...
if __name__ == '__main__':
    ...
    if len(count_list)>100:
        print('deliveroo爬虫正常,测试数据量为108,实际为%s'%(len(count_list)))
    else:
        print('deliveroo爬虫异常!!!,测试数据为108,实际为%s'%(len(count_list)))
